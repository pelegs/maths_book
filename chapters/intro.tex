...
We will switch to measuring angles in \emph{Radians} instead of degrees: $\theta$ radians is the length of an arc of a unit circle of angle $\theta$ (\xref{radians}). This allows us to use the same units as $x$ and $y$. The full circumference of a circle is of $2\pi$ radians, and therefore a single radian is equivalent to $\frac{180}{\pi} \approx 57.3$ degrees. \xref[tab]{rad_degs} shows some common angles in radians.
...
The use of the unit extension of the trigonometric functions as periodic functions $\mathbb{R}\longrightarrow\mathbb{R}$ with period $[\ang{0},\ang{360}]$.

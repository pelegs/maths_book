...
\subsection{Sets, subsets and cartesian products}
...
About 2500 years ago it was discovered that some numbers are not rational (and thus also not integers). The most famous example is the number $\sqrt{2}$ - there are not two integers $a,b$ such that $\frac{a}{b}=\sqrt{2}$. Such numbers are solutions to \emph{polynomial equations}, which we will not define yet (see section xxx). We will instead give an example for a 2nd order polynomial equation (called a \emph{quadratic equation}):
\begin{equation}
	x^{2} - 2x - 1 = 0.

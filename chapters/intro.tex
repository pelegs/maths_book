\pgfkeys{/pgfplots/Interval/.style={
		axis x line=middle,
		axis y line=none,
...
\begin{example}{Relations as subsets of Cartesian products}{relation}
	...
	then
	\begin{align*}
		A\times B = \left\{ (1,x),\ (1,y),\ (1,z), \right.\\
												(2,x),\ (2,y),\ (2,z),\\
												(3,x),\ (3,y),\ (3,z),\\
							  \left.	(4,x),\ (4,y),\ (4,z) \right\}.
	\end{align*}

	We can choose the following pairs to form a subset of $A\times B$:
	\[
		R = \left\{ (1,y),\ (2,x),\ (3,x),\ (3,y), (4,z)  \right\}.
	\]
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\node (x) at (2.5cm,1cm) {$x$};
			\node (y) at (2.5cm,0cm) {$y$};
			\node (z) at (2.5cm,-1cm) {$z$};
...
\begin{example}{Inverse relation}{inv_relation}
	...
	\[
		R^{-1} = \left\{ (y,1),\ (x,2),\ (x,3),\ (y,3), (z,4)  \right\}.
	\]
	Graphically:
	\begin{figure}[H]
		\centering
...
\begin{example}{Functions}{functions}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\node (x) at (2.5cm,-5.0cm) {$x$};
			\node (y) at (2.5cm,-6.0cm) {$y$};
			\node (z) at (2.5cm,-7.0cm) {$z$};
	...
	The pairs making up $f$ are $(1,x),\ (2,y),\ (3,z)$ and $(4,z)$, and the pairs making up $g$ are $(1,x),\ (2,z),\ (3,y)$ and $(4,x)$.
\end{example}

\begin{note}{Relations which are not functions}{}
	Note that the relation in \exampleref{relation} is \textbf{not} a function, since the element $3\in A$ is connected to more than one element in $B$, namely $x$ and $z$.
\end{note}
...
\end{equation}
$A$ is called the \emph{domain} of $f$, and $B$ its \emph{image}.

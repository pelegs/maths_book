...
An \emph{orthogonal projection} of a vector $\xr{\vec{u}}$ onto another vector $\xb{\vec{v}}$ essentially measures how much of $\xr{\vec{u}}$ is in the direction of $\xb{\vec{v}}$:
...
\begin{center}
  \begin{tikzpicture}[every node/.style={font=\Large}]
    ...
    \pgfmathsetmacro{\uL}{sqrt((\ux)^2+(\uy)^2)}
    \pgfmathsetmacro{\vx}{6}
    \pgfmathsetmacro{\vy}{2}
    \pgfmathsetmacro{\vL}{sqrt((\vx)^2+(\vy)^2)}
    \pgfmathsetmacro{\vxnormed}{\vx/\vL}
    \pgfmathsetmacro{\vynormed}{\vy/\vL}
    \pgfmathsetmacro{\projuv}{(\ux*\vx+\uy*\vy)/\vL}
    \pgfmathsetmacro{\projx}{\vxnormed*\projuv}
    \pgfmathsetmacro{\projy}{\vynormed*\projuv}
    \pgfmathsetmacro{\pux}{\projx-\ux}
    \pgfmathsetmacro{\puy}{\projy-\uy}
    \pgfmathsetmacro{\ph}{atan(\vy/\vx)}
    \pgfmathsetmacro{\dL}{0.35}

    % Coordinates
    \coordinate (u) at (\ux,\uy);
    \coordinate (v) at (\vx,\vy);
    \coordinate (vdir) at (\vxnormed*\dL,\vynormed*\dL);
    \coordinate (projection) at (\projx,\projy);
    \coordinate (perpdir) at ({\pux/sqrt((\pux)^2+(\puy)^2)*\dL},{\puy/sqrt((\pux)^2+(\puy)^2)*\dL});

    % Drawings
    \draw[vector, xred] (0,0) -- (u) node[midway, above left] {$\vec{u}$};
    \draw[vector, xblue] (0,0) -- (v) node[pos=1.05] {$\vec{v}$};
    \draw[dashed] (u) -- (projection);
    \draw ($(projection)-(perpdir)$) -- ++($-1*(vdir)$) -- ++(perpdir);
    \fill (0,0) circle (0.05);
    \draw[ultra thick, decorate, decoration={brace, amplitude=3pt, raise=5pt, mirror}] (0,0) -- (projection) node[midway, below, yshift=-10pt, rotate=\ph]{$\projection$};
  \end{tikzpicture}
\end{center}
...
\begin{center}
  \begin{tikzpicture}[every node/.style={font=\Large}]
    % Definitions
    \pgfmathsetmacro{\wx}{-3}
    \pgfmathsetmacro{\wy}{2}
    \pgfmathsetmacro{\wL}{sqrt((\wx)^2+(\wy)^2)}
    \pgfmathsetmacro{\vx}{6}
    \pgfmathsetmacro{\vy}{2}
    \pgfmathsetmacro{\vL}{sqrt((\vx)^2+(\vy)^2)}
    \pgfmathsetmacro{\vxnormed}{\vx/\vL}
    \pgfmathsetmacro{\vynormed}{\vy/\vL}
    \pgfmathsetmacro{\projwv}{(\wx*\vx+\wy*\vy)/\vL}
    \pgfmathsetmacro{\projx}{\vxnormed*\projwv}
    \pgfmathsetmacro{\projy}{\vynormed*\projwv}
    \pgfmathsetmacro{\pwx}{\projx-\wx}
    \pgfmathsetmacro{\pwy}{\projy-\wy}
    \pgfmathsetmacro{\ph}{atan(\vy/\vx)}
    \pgfmathsetmacro{\dL}{0.35}
    ...
    \coordinate (vdir) at (\vxnormed*\dL,\vynormed*\dL);
    \coordinate (projection) at (\projx,\projy);
    \coordinate (perpdir) at ({\pwx/sqrt((\pwx)^2+(\pwy)^2)*\dL},{\pwy/sqrt((\pwx)^2+(\pwy)^2)*\dL});

    % Drawings
    \draw[xblue, very thick, densely dotted] (0,0) -- ($-10*(vdir)$);
    \draw[vector, xorange] (0,0) -- (w) node[midway, above right] {$\vec{w}$};
    \draw[vector, xblue] (0,0) -- (v) node[pos=1.05] {$\vec{v}$};
    \draw[dashed] (w) -- (projection);
    \draw ($(projection)-(perpdir)$) -- ++($-1*(vdir)$) -- ++(perpdir);
    \fill (0,0) circle (0.05);
    \draw[ultra thick, decorate, decoration={brace, amplitude=3pt, raise=5pt}] (0,0) -- (projection) node[midway, below, yshift=-10pt, rotate=\ph]{$\projection[w][v][xorange]<0$};
  \end{tikzpicture}
\end{center}

Two more edge cases exist:
\begin{enumerate}
  \item If the two vectors are orthongonal, the projection is zero.
  \item If the two vectors are coliniear, the projection equals the projected vector.
\end{enumerate}

Since the two vectors form a right triangle together with the perpendicular line, we can use the angle $\ath$ between the vectors to calculate the projection:
...
\begin{center}
  \begin{tikzpicture}[every node/.style={font=\Large}]
    ...
    % Drawings
    \draw[xpurple, very thick, fill=xpurple!25] (0,0) -- ($4*(vdir)$) arc (\ph:\th:4) -- cycle;
    \draw[vector, xred] (0,0) -- (u) node[midway, above left] {$\vec{u}$};
    \draw[vector, xblue] (0,0) -- (v) node[pos=1.05] {$\vec{v}$};
    \draw[dashed] (u) -- (projection);
    \draw ($(projection)-(perpdir)$) -- ++($-1*(vdir)$) -- ++(perpdir);

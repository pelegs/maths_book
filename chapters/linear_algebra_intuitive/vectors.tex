...
A special kind of vector in $\Rs{3}$ is the so-called \emph{normal vector} to a plane $\mathbf{P}$: this vector, usually denoted as $\hat{n}_{\mathbf{P}}$, is pointing at the orthogonal direction to any vector of the plane (see XXX). Given one knows three points on the plane, its normal vector can be calculated. Let's see how: say we are given the following three points in $\mathbf{P}$ (for visualizing the following steps see YYY):
\begin{align}
	p &= (p_{x}, p_{y}, p_{z})\nonumber\\
	q &= (q_{x}, q_{y}, q_{z})\nonumber\\
	r &= (r_{x}, r_{y}, r_{z}),
...
\end{align}
(note: we use the superscripts to tell the points apart - don't get it confused with exponentiation!)
...
\begin{figure}
	...
	\begin{subfigure}{0.3\textwidth}
		\begin{tikzpicture}
			\begin{axis}[
				...
				\draw[vector] (0,0,0) -- (0,-1.2,3) node[pos=1.3] {$\hat{n}_{\mathbf{P}}$};
				...
				\node[text=green!50!black] at (4,3,3) {$\mathbf{P}$};
			\end{axis}
		\end{tikzpicture}
	...
	\end{subfigure}
	\caption{A normal vector $\hat{n}_{\mathbf{P}}$ to the plane $\mathbf{P}$.}
	\label{fig:normalVec}
\end{figure}

We can calculate two vectors lying on the plane by first considering the points as vectors, i.e.
\begin{align}
...
Normalizing $\vec{n}_{\mathbf{P}}$ will then yield the normal vector $\hat{n}_{\mathbf{P}}$\footnote{We leave this as a "challange" to the reader, due to laziness.}.


To wrap up this section, we present and solve a single promlem in the following example.

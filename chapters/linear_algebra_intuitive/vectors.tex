...
\begin{tikzpicture}
	\begin{axis}[
		...
		\draw[vector, xpurple] (0,0,0) -- (3,0.76,2.1) node[midway, below] {$\vec{r}$};
...
\begin{tikzpicture}
	...
	\draw[ultra thick, xblue] (2,0) -- (-2,0) node[above] {$\mathbf{P}$};
	\draw[vector] (0,0) -- (0,2.5) node[pos=1.1] {$\normalVec{n}{P}$};
	\draw[vector, xred] (-2,2) -- (0,0) node[pos=-0.1] {$\vec{d}$};
	\draw[vector, xred, densely dashed] (0,0) -- (2,-2) node[pos=1.1] {$\vec{d}$};
	\draw[vector, xpurple] (0,0) -- (2,2) node[pos=1.05] {$\vec{r}$};
\end{tikzpicture}

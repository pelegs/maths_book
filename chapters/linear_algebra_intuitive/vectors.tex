...
Let's take a vector, for example the vector $\vec{v}=\colvec{1;-3;7}$, and span it by three different basis sets:

\centering
\begin{tikzpicture}[node distance=2cm]
	\node (1-37) {$\colvec{1;-3;7}$};
	\node[above right of=1-37, xshift=3cm] (std) {$1\colvec{1;0;0} -3\colvec{0;1;0} +7\colvec{0;0;1}$};
	\node[below=of std.west, anchor=west] (b1) {$9\colvec{5;1;2} -23\colvec{0;1;0} -11\colvec{4;-1;1}$};
	\node[below=of b1.west, anchor=west] (b2) {$1.4\colvec{-1;0;2} +0.3\colvec{0;2;-3} +1.2\colvec{2;2;3}$};
	% !!!!!!!!!! LAST BASIS SET IS SOMEHOW WRONG - CHECK !!!!!!!!!! %

	\draw[-stealth, thick] (1-37.east) to [out=0, in=180, looseness=0.7] (std.west);
	\draw[-stealth, thick] (1-37.east) to [out=0, in=180, looseness=0.7] (b1.west);
	\draw[-stealth, thick] (1-37.east) to [out=0, in=180, looseness=0.7] (b2.west);
\end{tikzpicture}

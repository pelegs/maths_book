...
\end{equation}
The set $V$ represents a line in the direction of $\vec{v}$ going through the origin (see \autoref{fig:span of a single vector}). The set $V$ is itself a vector space of dimension $1$, and as such a \emph{subspace} of $\Rs{n}$. We say that it is the \emph{span} of the vector $\vec{v}$ (i.e. the $\vec{v}$ \emph{spans} the subspace $V$).
	...
	\begin{subfigure}[b]{0.47\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				...
				\tikzset{
					dline/.style={thick, dashed, black},
				}
				\coordinate (A) at ({ax}, {ay});
				\coordinate (B) at ({bx}, {by});
				\draw[vector, xred] (0,0) -- (A) node [above] {$\vec{v}$};
				\draw[dline] (A) -- (6,{f(6)});
				\draw[dline] (B) -- (-6,{f(-6)});
			\end{axis}
		\end{tikzpicture}
	...
	\begin{subfigure}[b]{0.47\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				...
				\draw[axisline] (0,-4,0) -- (0,4,0) node[pos=1.05] {$y$};
				\draw[axisline] (0,0,-4) -- (0,0,4) node[pos=1.05] {$z$};
				\draw[thick, dashed, black] (-2.67,1.34,4.0) -- (2.67,-1.34,-4.0);
				\draw[vector, xgreen] (0,0,0) -- (2,-1,-3) node[right] {$\vec{v}$};
				\addplot3[mesh, black, opacity=.075, domain=-4:4, samples=5] {0};
			\end{axis}
		\end{tikzpicture}
		\caption{$\Rs{3}$}
	\end{subfigure}
	\caption{The span of a single vector, shown as dashed lines: in $\Rs{2}$ (left) and $\Rs{3}$ (right).}
	\label{fig:span of a single vector}
\end{figure}

Similarily, the set of all linear combinations of two vectors $\vec{u},\vec{v}\in\Rs{n}$ that are not on the same line (i.e. there is no such $\alpha\in\mathbb{R}$ for which $\vec{v}=\alpha\vec{u}$), mathematically written as
\begin{equation}
	V = \left\{\alpha\vec{u}+\beta\vec{v} \mid \alpha,\beta\in\mathbb{R} \right\},
	\label{eq:span of a two vectors}
\end{equation}
is a plane that goes through the origin (see \autoref{fig:span of two vectors}).

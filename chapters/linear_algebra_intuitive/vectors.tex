...
\begin{center}
  \begin{tikzpicture}
    ...
    \coordinate (e1c) at (1,0.5);
    \coordinate (e2c) at (-0.5,1);
    \draw[vector, xred] (O) -- ++(e1c) node[midway, above] {$\vec{e}_{1}$};
    \draw[vector, xblue] (3,0) -- ++(e2c) node[midway, right] {$\vec{e}_{2}$};
  \end{tikzpicture}
\end{center}

We start with some vector $\xp{\vec{v}}$ and draw its components as well:

\begin{center}
  \begin{tikzpicture}
    \coordinate (vc) at ($(e1c)-1.5*(e2c)$);
    \draw[vector, xpurple] (O) -- ++(vc) node[midway, below left] {$\vec{v}$};
    \draw[vector, xred, dashed] (O) -- ++(e1c) node[midway, above left] {$\vec{e}_{1}$};
    \draw[vector, xblue, dashed] (e1c) -- ++($-1*(e2c)$) node[midway, above right] {$\vec{e}_{2}$};
    \draw[vector, xblue, dashed] ($(e1c)-(e2c)$) -- ++($-0.5*(e2c)$) node[midway, above right] {$\frac{1}{2}\vec{e}_{2}$};
  \end{tikzpicture}
\end{center}

We see that $\xp{1\cdot\vec{v}}=\xr{\vec{e}_{1}}-\xb{1.5\cdot\vec{e}_{2}}$, i.e. $\xp{\vec{v}}=\colvec{\xr{1};\xb{-1.5}}$. What then would be the components of, say, $2\xp{\vec{v}}$? First we scale $\xp{\vec{v}}$ by $\xp{2}$: remember, this just means "stretching" the vector by $\xp{2}$, keeping it pointing in the same direction:

\begin{center}
  \begin{tikzpicture}
    \draw[vector, xpurple] (O) -- ++(vc) node[midway, below left] {$\vec{v}$};
    \draw[vector, xpurple] (5,0) -- ++($2*(vc)$) node[midway, below left] {$2\vec{v}$};
    \draw[->, thick] (2.5,-0.5) -- ++(1.5,0) node[midway, above] {$2\times$};
  \end{tikzpicture}
\end{center}

Now we use the basis vectors $\xr{\vec{e}_{1}}$ and $\xb{\vec{e}_{2}}$ to get the column representation of $\xp{2\vec{v}}$:
\begin{center}
  \begin{tikzpicture}
    \draw[vector, xpurple] (O) -- ++($2*(vc)$) node[midway, below left] {$2\vec{v}$};
    \draw[vector, xred, dashed] (O) -- ++(e1c) node[midway, above left] {$\vec{e}_{1}$};
    \draw[vector, xred, dashed] (e1c) -- ++(e1c) node[midway, above left] {$\vec{e}_{1}$};
    \draw[vector, xblue, dashed] ($2*(e1c)$) -- ++($-1*(e2c)$) node[midway, above right] {$\vec{e}_{2}$};
    \draw[vector, xblue, dashed] ($2*(e1c)-(e2c)$) -- ++($-1*(e2c)$) node[midway, above right] {$\vec{e}_{2}$};
    \draw[vector, xblue, dashed] ($2*(e1c)-2*(e2c)$) -- ++($-1*(e2c)$) node[midway, above right] {$\vec{e}_{2}$};
  \end{tikzpicture}
\end{center}

We get that $\xp{2\vec{v}}=\xr{2\vec{e}_{1}}-\xb{3\vec{e}_{2}}$, i.e. $\xp{2\vec{v}}=\colvec{\xr{2};\xb{-3}}=\colvec{\xr{2\cdot1};\xb{2\cdot(-1.5)}}$ - scaling $\xp{\vec{v}}$ by $\xp{2}$ simply multiplied both its components by $\xp{2}$.

...
Expanding this idea to higher dimensions, any three vectors $\vec{u},\vec{v},\vec{w}\in\Rs{n}$ that are not co-planar (i.e. that don't lie on the same plane) span a 3-dimensional subspace of $\Rs{n}$.
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% Vectors
			\draw[vector, xgreen!75!gray] (0,0,0) -- (-2,3,1.2);
			\draw[vector, xgreen!75!gray] (0,0,0) -- (+3,3,1.2);
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Two vectors (colored green) span a plane (also colored green) in $\Rs{3}$. The $xy$-plane (i.e. $z=0$) is shown in blue.}

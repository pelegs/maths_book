...
Any vector can be \emph{decomposed} into its projection on $n$ orthogonal directions. In fact, this is exactly what we do when we write a vector as a linear combination of basis set vectors: consider the vector
\[
	\vec{v} = \colvec{v_{1};v_{2};\vdots;v_{n}}.
\]
...
\begin{equation}
	...
	\label{eq:}
\end{equation}
and thus the component $v_{i}\eb{i}=\left(\proj{\eb{i}}{\vec{v}}\right)\eb{i}$ is in itself a vector of norm $v_{i}$ pointing at the direction $\en{i}$.
...
\begin{tikzpicture}
	\begin{axis}[
		...
		\draw[vector, xred] (-3,-2,1) -- (0,0,0) node[midway, below] {$\vec{d}$};
...
\begin{tikzpicture}
	...
	\draw[thick, xpurple, fill=xpurple!20] (0,0) -- (0,1) arc (90:45:1);
	\node[xred] at (-0.25,0.6) {$\theta$};
	\node[xpurple] at (0.25,0.6) {$\theta$};
	\draw[ultra thick, xblue] (2.0,0) -- (-2.0,0) node[above] {$\mathbf{P}$};
	\draw[vector] (-0.0,0) -- (-0.0,2.5) node[pos=1.1] {$\normalVec{n}{P}$};
	\draw[vector, xred] (-2.0,2) -- (-0.0,0) node[pos=-0.1] {$\vec{d}$};
	\draw[vector, xpurple] (-0.0,0) -- (2.0,2) node[pos=1.05] {$\vec{r}$};
\end{tikzpicture}

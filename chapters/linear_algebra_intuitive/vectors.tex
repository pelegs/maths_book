...
\begin{center}
  \begin{tikzpicture}[]
    ...
    \fill[xorange!25!gray] (vec2d) circle (0.2) node[above] {$\bm{p}=(p_{x},p_{y})$};
    \draw[xorange!25!gray, dashed] (vec2d) -- (\px,0);
    \draw[xorange!25!gray, dashed] (vec2d) -- (0,\py);
    \draw[vector, xorange] (0,0) -- (vec2d) node[midway, above] {$\vec{u}$};
    \fill (0,0) circle (0.2) node[] (O2) {};
    \end{axis}
    \node[below left of=O2] {$\bm{O}=(0,0)$};
  \end{tikzpicture}
\end{center}
...
\begin{center}
  \begin{tikzpicture}[]
    ...
    \draw[vector, xorange] (0,0) -- (vec2d) node[midway, above] {$\vec{u}$};
    \draw[vector, xred] (0,0) -- (4,0) node[midway, below] {$\hat{x}$};
    \draw[vector, xblue] (0,0) -- (0,4) node[midway, left] {$\hat{y}$};
    \fill (0,0) circle (0.2) node[] (O2) {};
    \end{axis}
  \end{tikzpicture}
\end{center}

Using the standard basis set, the first component of any vector is simply $p_{x}$ and the second component is $p_{y}$, i.e. $\xo{\vec{u}}=\xtwocolvec{p_{x}}{p_{y}}$. For example, we see that the vector $\xo{\vec{u}}$ in the above figure has the column represented $\xo{\vec{u}}=\xtwocolvec{2}{1}$.

\section{Vectors}
\begin{motivation}{Vectors}{motivating_vectors}
    Say we wish to write a simple simulation gas particles inside of a container. We will model the particles as simple "hard" balls - i.e. similar to billiard balls which can't be deformed, and interact with eachother via simple collisions. The container will be modeled as a simple enclosed space made up of planar walls, for example:
    ...
    \begin{center}
        ...
        \begin{tikzpicture}[
            ...
            \pgfmathsetmacro{\L}{5}
            \pgfmathsetmacro{\R}{1}
            \pgfmathsetmacro{\r}{\R*0.25}\def\Rad{\r cm}
            \pgfmathsetmacro{\N}{25}
            \tikzset{
                wall/.style={very thick, draw=black, fill=xblue, fill opacity=0.1},
                sphere/.style={ball color=#1, #1, opacity=1},
            }
            % Back walls
            \fill[wall] (0,0,0) -- (0,\L,0) -- (\L,\L,0) -- (\L,0,0) -- cycle;
            \fill[wall] (0,\L,0) -- (0,\L,\L) -- (\L,\L,\L) -- (\L,\L,0) -- cycle;
            \fill[wall] (0,0,0) -- (0,0,\L) -- (0,\L,\L) -- (0,\L,0) -- cycle;
            % Spheres
            \foreach \i in {1,2,...,\N}{
                \pgfmathsetmacro{\X}{random(\R,\L-\R)}
                \pgfmathsetmacro{\Y}{random(\R,\L-\R)}
                \pgfmathsetmacro{\Z}{random(\R,\L-\R)}
                \shade[sphere={xpurple!85!white}] (\X,\Y,\Z) circle (\Rad);
            }
            % Front walls
            \fill[wall] (0,0,\L) -- (0,\L,\L) -- (\L,\L,\L) -- (\L,0,\L) -- cycle;
            \fill[wall] (0,0,0) -- (0,0,\L) -- (\L,0,\L) -- (\L,0,0) -- cycle;
            \fill[wall] (\L,0,0) -- (\L,0,\L) -- (\L,\L,\L) -- (\L,\L,0) -- cycle;

            % Axes
            \pgfmathsetmacro{\st}{-1}
            \draw[vector, xred]   (\st,\st,0) -- ++(1,0,0) node[pos=1.1] {$x$};
            \draw[vector, xblue]  (\st,\st,0) -- ++(0,2,0) node[pos=1.4] {$y$};
            \draw[vector, xgreen] (\st,\st,0) -- ++(0,0,1) node[pos=1.2] {$z$};
            \shade[sphere={black}] (\st,\st) circle (0.5mm);
        \end{tikzpicture}
    \end{center}
...
% \end{example}
%
% \tbw{write an example involving normalization?}
%
...
%     \item orthogonal and orthonormal basis sets
%   \end{enumerate}
% }
%
...
% \end{equation}
%
% This probably looks somewhat daunting, so let's take it step-by-step: the component $i$ of the resulting vector $\vec{c}=\vec{a}\cross\vec{b}$ is
% \begin{equation}
...
%   \tikzset{
%     every path/.style={very thick, decorate, decoration={brace, amplitude=3pt, raise=-3pt}},
%     every node/.style={midway, above},
%   }
%   \draw($(pic cs:j1s)+(0,7mm)$) -- ($(pic cs:j1e)+(0,7mm)$) node{$j=1$};
%   \draw($(pic cs:j2s)+(0,7mm)$) -- ($(pic cs:j2e)+(0,7mm)$) node{$j=2$};
%   \draw($(pic cs:j3s)+(0,7mm)$) -- ($(pic cs:j3e)+(0,7mm)$) node{$j=3$};
% }
%

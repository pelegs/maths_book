...
\begin{example}{Real vectors}{real vectors}
	The vectors from the previous example, translated (moved) such that their origins all lie on the same point:
...
In order to be able to use vectors for actual calculations we must somehow quantify their properties. When quantifying geometric shapes we often first define some unit of measurement, for example a centimeter (\si{cm}). We then use this unit to measure the length of different objects (\autoref{fig:unit_geometry}).

While this simple approach works well for describing lengths, in the case of vectors we also want to quantify directions - which becomes a bit complicated in higher dimensions if we use angles. Instead, we use more than one unit of measurement; in fact, we use a unit of measurement for each dimension, which we call a \emph{unit vector}. For example, we can choose the following two vectors $\color{xred}{\vec{e}_{1}}$ and $\color{xblue}{\vec{e}_{2}}$ to be our unit vectors:

% !!!!
% Write: measuring vectors using these unit vectors

\centering
\begin{tikzpicture}
  \draw[vector, xred] (0,0) -- (1,0.5) node[midway, above] {$\vec{e}_{1}$};
  \draw[vector, xblue] (2,0) -- (2.3,0.6) node[midway, left] {$\vec{e}_{2}$};
\end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \tikzset{
      unit/.style={very thick, |-|, xred},
    }
    \draw[unit] (-3,0) -- (-2,0) node[midway, above] (unit) {$d$};
    \node[below of=unit, anchor=center] {Unit length};

    % Rectangle
    \draw[very thick, xblue] (0, -2) rectangle (2,-1);
    \draw[unit] (0,-0.7) -- (1,-0.7);
    \draw[unit] (1,-0.7) -- (2,-0.7);
    \draw[unit] (2.3,-1) -- (2.3,-2);
    \node[anchor=center] at (1,-2.5) {Perimeter: $6\cdot d$};

    % Triangle side
    \draw[very thick, xpurple] (1,1) -- (2.8,1) -- (2.8,3.4) -- cycle;

    % !!!!!!!
    % Continue figure!!
  \end{tikzpicture}
  \caption{Using a defined unit measurement to measure the lengths of different geometric objects.}
  \label{fig:unit_geometry}
\end{figure}

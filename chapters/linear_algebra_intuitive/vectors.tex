...
Vectors have different components when using different basis vectors. For example, we can use the following two vectors $\xdg{\vec{d}_{1}}$ and $\xo{\vec{d}_{2}}$ as basis vectors:
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
and its components are $\xdg{v^{1}}=2$ and $\xo{v^{2}}=2.5$.
...
%       ytick={-2,-1,1,2},
%     ]
%     \draw[vector, xred] (0,0) -- (1,0) node[midway, above] {$\hat{x}$};
%     \draw[vector, xblue] (0,0) -- (0,1) node[midway, left] {$\hat{y}$};
%     \end{axis}
%   \end{tikzpicture}
%   \caption{The two standard basis vectors $\hat{x}$ and $\hat{y}$ drawn in a Cartesian coordinate system.}
%   \label{fig:standard_basis_R2}
% \end{figure}

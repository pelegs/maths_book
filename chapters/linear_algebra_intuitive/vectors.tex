...
The main difference between vectors in $\Rs[2]$ and vectors in $\Rs[3]$ is that any basis set in $\Rs[3]$ must contain \textbf{three} basis vectors instead of two, and therefore the column representation of vectors in $\Rs[3]$ has three entries. For example, given the three basis vectors $\xr{\vec{e}_{1}},\ \xb{\vec{e}_{2},\ \xdg{\vec{e}_{3}}}$, then
\[
...
\tbw{box's diagonal line, Pythagorean theorem in 3D bla bla bla.}

...
\begin{center}
  \begin{tikzpicture}
    ...
    \coordinate (e2) at (-0.2,1);
    \draw[vector, xred] (O) -- ++(e1) node[midway, above] {$\vec{e}_{1}$};
    \draw[vector, xblue] (3,0) -- ++(e2) node[midway, left] {$\vec{e}_{2}$};
  \end{tikzpicture}
\end{center}
...
\begin{center}
  \begin{tikzpicture}
    \draw[vector, xpurple] (O) -- ++($(e1)+2*(e2)$) node[midway, left] {$\vec{v}$};
    \draw[vector, xred, dashed] (O) -- ++(e1) node[midway, above] {$\vec{e}_{1}$};
    \draw[vector, xblue, dashed] (e1) -- ++(e2) node[midway, right] {$\vec{e}_{2}$};
    \draw[vector, xblue, dashed] ($(e1)+(e2)$) -- ++(e2) node[midway, right] {$\vec{e}_{2}$};
    \fill (O) circle (0.05);
  \end{tikzpicture}
\end{center}

We see that using these basis vectors, $\xp{\vec{v}}=\xr{\vec{e}_{1}}+2\xb{\vec{e}_{2}}$. This means that we need to add a single $\xr{\vec{e}_{1}}$ and two times $\xb{\vec{e}_{2}}$ to construct $\xp{\vec{v}}$. We denote this fact by writing $\xp{\vec{v}}$ as a column of two numbers:
\[
  \xp{\vec{v}}=\colvec{\xr{1};\xb{2}}.
\]

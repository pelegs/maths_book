...
% The standard basis set vectors $\cxhat,\ \cyhat$ and $\czhat$ have a nice relationship in regards to the cross product: since they form an orthonormal basis set, the cross product of any two of the vectors $\hat{e}_{\alpha},\ \hat{e}_{\beta}$ (where $\alpha,\beta\in\left\{\xr{x},\xb{y},\xdg{z}\right\}$) is either the third vector (up to a sign) or zero (in case $\alpha=\beta$). So for example, $\cxhat\times\cyhat=\czhat$ and $\cyhat\times\cyhat=\vec{0}$.

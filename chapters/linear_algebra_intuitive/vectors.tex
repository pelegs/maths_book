...
Vectors can be decomposed to their components, the number of components depending on the dimension of space the vectors exist in: 2-dimensional vectors can be decomposed to 2 components, 3-dimensional vectors can be decomposed to 3 components, etc.

To decompose a vector, we first need to choose a coordinate system: the most commonly used system, and the one we will use for most of this chapter, is the Cartesian coordinate system. We place a vector $\vec{v}$ in the system such that its origin lies at the origin of the system. We then draw a line from its head to each of the axes in the system, such that the lines intercept the respective axis at $\ang{90}$ (see \autoref{fig:vector components}), the point of interception on each axis (labeled $v_{x},v_{y},v_{z}$ - or more generally $v_{1},v_{2},v_{3},\dots$) is the component of the vector in that axis. The vector can then be written as a column with its components:
\begin{equation}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Some vectors on a cartesian coordinate system, showing their $x$- and $y$-components.}
	\label{fig:parallelogram}
\end{figure}

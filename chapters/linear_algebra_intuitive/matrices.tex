...
% \begin{align*}
% 	\Amatv{Av1}\cdot\Vvec{vv1} &= \begin{bNiceMatrix}[name=u1]-4\\\textcolor{black!20}{16}\\\textcolor{black!20}{24}\\\textcolor{black!20}{26}\end{bNiceMatrix},\\[5mm]
% 	\Amatv{Av2}\cdot\Vvec{vv2} &= \begin{bNiceMatrix}[name=u2]\textcolor{black!20}{-4}\\16\\\textcolor{black!20}{24}\\\textcolor{black!20}{26}\end{bNiceMatrix},\\[5mm]
% 	\Amatv{Av3}\cdot\Vvec{vv3} &= \begin{bNiceMatrix}[name=u3]\textcolor{black!20}{-4}\\\textcolor{black!20}{16}\\24\\\textcolor{black!20}{26}\end{bNiceMatrix},\\[5mm]
% 	\Amatv{Av4}\cdot\Vvec{vv4} &= \begin{bNiceMatrix}[name=u4]\textcolor{black!20}{-4}\\\textcolor{black!20}{16}\\\textcolor{black!20}{24}\\26\end{bNiceMatrix}.
% \end{align*}
% \tikz[overlay, remember picture, blend mode=multiply]{
% 	\node[above of=vv1-1-1] (vv1-txt) {$\vec{v}$};
% 	\draw[-stealth] (vv1-txt) -- ($(vv1-1-1)+(0,10pt)$);
% 	\foreach \r in {1,...,4}{
% 		\node[mathl={xcol\r!20}, draw=xcol\r, rounded corners, fit=(Av\r-\r-1)(Av\r-\r-3)] {};
% 		\node[mathl={xcol\r!20}, draw=xcol\r, rounded corners, fit=(vv\r-1-1)(vv\r-3-1)] {};
% 		\node[mathl={xcol\r!20}, draw=xcol\r, rounded corners, fit=(u\r-\r-1)(u\r-\r-1)] {};
% 		\node[xcol\r, left of=Av\r-\r-1, anchor=east] (Av\r-txt) {\nth{\r} row of $A$};
% 		\draw[-stealth, xcol\r] (Av\r-txt) -- ($(Av\r-\r-1.west)+(-7pt,0)$);
% 		\node[xcol\r, right of=u\r-\r-1, anchor=west] (u\r-txt) {\nth{\r} element of $\vec{u}$};
% 		\draw[-stealth, xcol\r] (u\r-txt) -- ($(u\r-\r-1.east)+(7pt,0)$);
% 	}
% }
...
\[
	...
	\begin{bNiceMatrix}[name=u1]
		1\cdot2+0\cdot5+2\cdot0\\
		0\cdot2+3\cdot5+1\cdot0\\
		2\cdot2+4\cdot5+0\cdot0\\
		6\cdot2+1\cdot5+3\cdot0\\
	\end{bNiceMatrix}
	\quad\quad
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\matrix [mat] (top) at (3.5,3.7) {
			|[draw, fill=xred!30]|b_{11} & b_{12} & |[draw, fill=xblue!30]|b_{13}\\
			|[draw, fill=xred!30]|b_{21} & b_{22} & |[draw, fill=xblue!30]|b_{23}\\
		};

		\matrix [mat, text=white] (center) at (3.5,0) {
			|[draw, black, fill=xred!30]|c_{11} & 0 & 0 \\
			0 & 0 & 0 \\
			0 & 0 & |[draw, black, fill=xblue!30]|c_{33} \\
			0 & 0 & 0 \\
		};
		...
		\draw[boxarr, xblue] (left-3-2) -- (center-3-3);
		\draw[boxarr, xred] (top-2-1) -- (center-1-1);
		\draw[boxarr, xblue] (top-2-3) -- (center-3-3);
		...
		\draw[line width=3pt, xblue] (top-1-3.north west) rectangle (top-2-3.south east);
	\end{tikzpicture}

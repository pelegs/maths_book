...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{The same complex number $z$ from \figref{complex number} shown with its polar components $r=|z|=\sqrt{a^{2}+b^{2}}$ and $\theta=\arctan\left( \frac{b}{a} \right)$.}
...
\end{align}
which is indeed the result seen in \eqref{complex_product}. We can also develope further the second row of \eqref{complex_product_geometric} using some trigonometry (specifically the trigonometric identities in \eqref{trig product to sum}):
\begin{align}

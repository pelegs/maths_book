...
In \tabref{complex_exponentials} we can see some usefull complex exponentials $e^{xi}$. Specifically, using the result for $x=\pi$ one gets the famous \emph{Eurler's identity}, considered by many to be one of the most beautiful equations in mathematics, as it binds together five important numbers: $0,1,\pi,e$ and $i$:
\begin{equation}
	e^{\pi i} + 1 = 0.

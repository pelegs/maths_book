...
\end{align}
which is indeed the result seen in \eqref{complex_product}.

In the geometric form the complex conjugate of a number $z=r\left[ \cos\left( \theta \right) +i\sin\left( \theta \right) \right]$ is
\begin{equation}
	\conj{z} = r\left[ \cos(\theta) -i\sin\left( \theta \right) \right],
	\label{eq:complex_conjugation_geometric}
\end{equation}
and it is easy to show that it corresponds to \eqref{complex_conjugation} (see practice question xxx).
...
\begin{align}
	...
	&= r^{2}\left[ \cos\left( \theta \right)^{2} -\cancel{i\cos\left( \theta \right)\sin\left( \theta \right)} +i\cancel{\sin\left( \theta \right)\cos(\theta)} + \sin\left( \theta \right)^{2}\right]\nonumber\\
	&= r^{2}\left[ \sin\left( \theta \right)^{2} + \cos\left( \theta \right)^{2} \right]\nonumber\\
	&= r^{2} = a^{2}+b^{2}.
	\label{eq:conjugate_product_geometric}
\end{align}

In 1748 Leonhard Euler published his famous work \textit{Introductio in analysin infinitorum}\footnote{Latin for \textbf{Introduction to the Analysis of the Infinite}.}. In it he introduced the following relation:
\begin{equation}
	e^{ix} = \sin(x) + i\cos(x).

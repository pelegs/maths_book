...
An important family of shapes in mathematics are the \emph{conic sections}, which arise as the intersection of a plane and an infinite ``right circular'' cone (let's call it \textit{RC cone} for simplicity). An RC cone is a 3D shape which is created when a right triangle is rotating by $\ang{360}$ around the triangle's height (see \autoref{fig:RC_cone}).
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Rotating a right triangle by $\ang{360}$ around it height $h$, yielding a right circular cone with radius $r$ and angle $\theta$.}
  \label{fig:RC_cone}
\end{figure}

...
\begin{figure}
	...
	\begin{tikzpicture}[every node/.style={font=\small}]
		...
		\foreach \th in {0, 45, 90, 135, 180, 200, 270, 330}{
			\draw[densely dashed, xdarkpurple] (0,0) -- ({1.05*\R*cos(\th)},{1.05*\R*sin(\th)});
...
\subsection{Graphs}
As seen previosuly, the functions $\sin(x)$ and $\cos(x)$ are periodic, having both the period $T=2\pi$. Their graphs are depicted in \figref{sin and cos graphs}. The value of $\sin(x)$ is always equal to that of $\cos\left(x-\frac{\pi}{2}\right)$: we say that the two functions have a \emph{phase difference} of $\pi/2$. The graph of $\tan(x)$ is depicted in \figref{tan graph}.
